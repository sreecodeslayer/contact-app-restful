...
phases:
  ...
  post_build:
    commands:
      ...
      - curl  -O -L "-H Authorization:token $GITHUB_TOKEN" -H "Accept:application/vnd.github.v3.raw" https://api.github.com/repos/sreecodeslayer/contact-app-restful/contents/infra/kubernetes/deployment.yaml
      - |
